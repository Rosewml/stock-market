...
- 应用只支持韩国股票。
- 搜索输入仅支持股票名称，不支持股票代码或其他兜底输入方式。
- 用户最多同时选中 9 只股票。
- 已选择的 9 只或 9 只以内股票，需要展示在同一张图片中。
- 当已选数量达到 9 只后，继续勾选其他股票时，需要弹出消息提示，并且不允许本次勾选生效。
- 当前价格发生变化后，需要自动重新生成图片。
- 视觉主题以蓝色主题为准。
- 手机 UI 样式保持当前现有的 3 种方案，不再新增其他风格。
- 打包后的 `.exe` 双击启动时允许先出现 `cmd` 窗口，不需要静默运行；如果发生报错，直接在控制台窗口展示错误信息即可。
